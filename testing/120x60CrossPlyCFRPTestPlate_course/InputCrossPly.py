...
print("Geom Size %dx%dx%d" % (NX, NY, NZ))
...
# Geom[int(d_cx/dx-d_wx/dx):int(d_cx/dx+d_wx/dx),int(d_cy/dy-d_wy/dy):int(d_cy/dy+d_wy/dy),24] = 0
# planeCounts = np.sum(np.sum(Geom, axis=2), axis=1).astype(dtype=np.uint64)
...
if(plotting):
    ...
    plt.figure(1)
    plt.imshow(Geom[:, :, int(NZ/2)], interpolation='none')
    # plt.figure(2)
...
Rot.tofile('Rotations1.bin', sep='')
rot_array.tofile('Rotations.bin', sep='')
